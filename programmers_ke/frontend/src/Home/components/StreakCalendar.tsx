...
export const StreakCalendar = ({ events }: StreakCalendarProps) => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        const eventForDay = events && Array.isArray(events) ? events.find(event => dayjs(event.date).isSame(date, 'day')) : undefined;
        ...
        days.push(
            <div key={date.format('YYYY-MM-DD')} className={`text-center text-sm font-adventpro 
                                                 ${isToday ? 'bg-primary rounded-full text-white dark:bg-secondary dark:text-black' : 
    ...
    const Days = (props: { children: React.ReactNode }) => {
        return (
           <div className="text-center font-novaflat text-sm
